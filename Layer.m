...
@synthesize layerID,name,width,height;

-(id)initWithName:(NSString *)aName 
          layerID:(int)aLayerID 
       layerWidth:(int)aLayerWidth 
      layerHeight:(int)aLayerHeight
{
    if(self = [super init]) {
        layerID = aLayerID;
        width = aLayerWidth;
    ...
    tileImages = calloc(width * height, sizeof(TexturedColoredQuad));
    
    return self;
}

-(void)addTileAt:(CGPoint)aTileCoord 
       tileSetID:(int)aTileSetID 
          tileID:(int)aTileID 
        globalID:(int)aGlobalID 
           value:(int)aValue
{
...
{
    int index = (int)(width * aPoint.y) +  (int)aPoint.x;
